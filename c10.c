...
struct table *atable;
{
	...
	/* BCD: Some special cases are checked first, which don't fit into the
	 * table scanning approach nearly. */
	switch (tree->op)  {
	...
	case CALL:
		/* BCD: CALL/MCALL is used in pass 1.  This is converted to CALL1 (for
...
struct table *table;
{
	...
	if(c==QUEST) {
		/* BCD: Handle the ternary operator; the tables only work up to binaries. */
		if (table==cctab)
			return(-1);
	 ...
	 * added to the compiler for experimenting with new rules (possibly
	 * written in C and not assembler generated by cvopt?)
	 */
	if (table==cregtab)
		table = regtab;
